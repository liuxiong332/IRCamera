...
HRESULT CTxtWinHost::CreateTextServices(IUnknown *punkOuter, ITextHost *pITextHost, IUnknown **ppUnk)
{
  typedef HRESULT(_stdcall *CTSFunc)(IUnknown *punkOuter, ITextHost *pITextHost, IUnknown **ppUnk);
  CTSFunc ctsFunc = NULL;
  hRichEditModule = LoadLibrary(_T("Riched20.dll"));

  if (NULL != hRichEditModule)
  {
    ctsFunc = (CTSFunc)GetProcAddress(hRichEditModule, "CreateTextServices");

    if (NULL != ctsFunc)
      return ctsFunc(punkOuter, pITextHost, ppUnk);
  }
  return S_FALSE;
}
